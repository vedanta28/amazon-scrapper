...
url = 'https://www.amazon.in/WONDERLAND-FOODS-DEVICE-Raisins-Kishmish/dp/B08VGDWJRM?ref_=Oct_DLandingS_D_07408eea_0'
...
def get_MRP(soup):
    try:
        MRP = price = soup.find("span", attrs={"class":'a-price a-text-price a-size-base'}).span.text
    except AttributeError:
        MRP = ""
    return MRP
def get_rating(soup):
    try:
        rating = rating_section.find("span", attrs={"class":'a-size-base a-color-base'}).text
    except AttributeError:
        rating = ""
    return rating
